...
router.post("/signup", (req, res) => {
  ...
  */
  ...
  User.findOne({ username }).then((found) => {  // Search the database for a user with the username submitted in the form

    if (found) {return res.status(400).json({ errorMessage: "Username already taken." });}
    // If the user is found, send the message username is taken

    return bcrypt    // if user is not found, create a new user - start with hashing the password
      .genSalt(saltRounds)
      .then((salt) => bcrypt.hash(password, salt))
      .then((hashedPassword) => {
        return User.create({    // Create a user and save it in the database
          email,
          username,
  ...
  User.findOne({ username })
    .then((user) => {
      ...
      bcrypt.compare(password, user.password).then((isSamePassword) => {
        ...
        const payload = { _id, email, username, createdAt };
